...
def segment(image, mu=0.15, lambda1=1, lambda2=1, tol=1e-3, max_iter=500, dt=0.5, extended_output=False):
    ...
    grayScale_aux = grayScale.copy()
    grayScale_aux[circle_mask]=0

    segmented, phi, energies = chan_vese(grayScale_aux, mu, lambda1, lambda2, tol, max_iter,
    ...
    out_aux = binary_opening(out).astype(int)

    if extended_output:
        return grayScale, grayScale_aux, out, out_aux
    else:
        return out
